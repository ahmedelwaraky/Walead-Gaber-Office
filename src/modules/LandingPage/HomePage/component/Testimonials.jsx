...
export default function Testimonials() {
  ...
  // 3 cards for desktop, 1 for mobile
  const desktopCards = [
    items[current % total],
    items[(current + 1) % total],
    items[(current + 2) % total],
  ];
  const mobileCard = items[current % total];
  ...
  const handleTouchEnd = (e) => {
    ...
    if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 40) {
      // rtl: swipe left = prev, swipe right = next (and vice-versa for ltr)
      if (dir === "rtl") { dx > 0 ? prev() : next(); }
      else               { dx < 0 ? next() : prev(); }
    }
  ...
  const Card = ({ item, active = false, delay = 0 }) => (
    <div
      key={`${item.id}-${current}`}
      className={`testimonial-card rounded-3xl p-6 border relative ${active ? "active" : ""}`}
      style={{
    ...
    >
      <div
        className="absolute top-5 left-5 text-4xl font-black select-none"
        style={{ color: "rgba(212,168,67,0.13)", fontFamily: "Georgia,serif", lineHeight: 1 }}
      >"</div>
      ...
      <div className="flex items-center gap-3">
        <div
          className="avatar-gold w-10 h-10 rounded-full flex items-center justify-center text-sm font-black shrink-0"
          style={{ color: "#050d1f" }}
        >
          {item.letter}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="testimonial-orb absolute rounded-full"
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-10">
        ...
        {/* Header */}
        <div className={`text-center mb-10 md:mb-12 ${visible ? "testimonial-reveal" : "opacity-0"}`}>
          <div
          ...
          <h2 className="font-black mb-3" style={{ fontSize: "clamp(24px,4vw,44px)", color: "#fff" }}>
        ...
        {/* ── MOBILE SLIDER ── */}
        <div
          className={`md:hidden ${visible ? "testimonial-reveal" : "opacity-0"}`}
          style={{ animationDelay: "0.2s" }}
          onTouchStart={handleTouchStart}
          onTouchEnd={handleTouchEnd}
        >
          <Card item={mobileCard} active />

          {/* Mobile controls row */}
          <div className="flex items-center justify-between mt-5 px-1">
            <NavBtn onClick={dir === "rtl" ? next : prev}>
            ...
            <NavBtn onClick={dir === "rtl" ? prev : next}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
        ...
        {/* ── DESKTOP SLIDER ── */}
        <div
          className={`hidden md:flex items-center gap-4 ${visible ? "testimonial-reveal" : "opacity-0"}`}
          style={{ animationDelay: "0.2s" }}
        >
          <NavBtn onClick={prev}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
          ...
          <div className="flex-1 grid grid-cols-3 gap-5">
            {desktopCards.map((item, i) => (
              <Card key={`${item.id}-${current}-${i}`} item={item} active={i === 1} delay={i * 0.07} />
            ))}
          </div>
