...
function StatBox({ n, suffix, label }) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.07)", transition: "all 0.28s ease" }}
      onMouseEnter={(e) => { e.currentTarget.style.background = "rgba(212,168,67,0.07)"; e.currentTarget.style.borderColor = "rgba(212,168,67,0.22)"; e.currentTarget.style.transform = "translateY(-3px)"; }}
    ...
    >
      <div className="text-2xl font-black text-white leading-none mb-1">
        {val}
        <span className="text-lg" style={{ color: "#d4a843" }}>{suffix}</span>
      </div>
...
export default function HeroSection() {
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(180deg,#040c1d 0%,#070f22 50%,#050d1f 100%)", height: "100vh", overflow: "hidden" }}
    >
      {/* ── BG decorations ── */}
      ...
      <div className="relative flex-1 flex items-center" style={{ zIndex: 1 }}>
        <div className="w-full max-w-7xl mx-auto px-6 md:px-10 grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-14 items-center" style={{ paddingTop: "5.5rem", paddingBottom: "2rem" }}>
          {/* TEXT SIDE */}
          <div>
            <div className="h-fr d1 inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full text-sm font-medium mb-5 border" style={{ background:"rgba(212,168,67,0.09)",borderColor:"rgba(212,168,67,0.28)",color:"#f0c96a" }}>
            ...
            <h1 className="h-fr d2 font-black leading-snug mb-4" style={{ fontSize: "clamp(28px,4vw,52px)" }}>
              <span style={{ background:"linear-gradient(135deg,#d4a843 0%,#f0c96a 50%,#faeab3 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text" }}>{t("hero.headline1")}</span>
              <br />
              <span className="text-white">{t("hero.headline2")}</span>
              <br />
              <span style={{ WebkitTextStroke:"2px rgba(255,255,255,0.22)",color:"transparent" }}>{t("hero.headline3")}</span>
            </h1>
            <p className="h-fr d3 text-base leading-7 mb-6 max-w-xl font-normal" style={{ color: "rgba(255,255,255,0.58)" }}>{t("hero.sub")}</p>
            <div className="h-fr d4 flex flex-wrap items-center gap-3.5 mb-6">
              <button className="hcta-gold inline-flex items-center gap-2.5 px-8 py-3.5 rounded-xl text-base font-bold border-0 cursor-pointer" onClick={() => document.getElementById("services")?.scrollIntoView({ behavior:"smooth" })}>
                {t("hero.cta_primary")}
              ...
              <a href="https://wa.me/201066335201" target="_blank" rel="noopener noreferrer">
          ...
          <div className="h-fl d3 relative hidden xl:block">
            <div className="rounded-3xl p-8 border relative overflow-hidden" style={{ background:"rgba(255,255,255,0.04)",borderColor:"rgba(255,255,255,0.09)",backdropFilter:"blur(20px)" }}>
              <div className="absolute top-0 right-0 w-52 h-52 rounded-full pointer-events-none" style={{ background:"radial-gradient(circle,rgba(212,168,67,0.08) 0%,transparent 70%)",transform:"translate(30%,-30%)" }} />
              <div className="flex items-center justify-between mb-6 pb-5" style={{ borderBottom:"1px solid rgba(255,255,255,0.07)" }}>
                ...
                <span className="flex items-center gap-1.5 text-xs font-bold" style={{ color:"#4ade80" }}>
                  <span className="h-pulse w-1.5 h-1.5 rounded-full bg-green-400" />
                  {t("hero.card_live")}
                </span>
              </div>
              <div className="mb-6">
                ...
                <div className="inline-flex items-center gap-1 text-xs font-bold rounded-full px-3 py-1 mt-2" style={{ color:"#4ade80",background:"rgba(74,222,128,0.1)" }}>
      ...
      <div className="relative px-6 md:px-10 pb-5" style={{ zIndex: 1 }}>
        <div className="max-w-7xl mx-auto rounded-2xl border grid" style={{ background:"rgba(255,255,255,0.04)",borderColor:"rgba(255,255,255,0.08)",backdropFilter:"blur(20px)",gridTemplateColumns:`repeat(${STATS.length}, 1fr)` }}>
          {STATS.map(({ n, suffix, label }, i) => (
            <div key={label} className="flex flex-col items-center py-4 px-4" style={{ borderRight: i < STATS.length - 1 ? "1px solid rgba(255,255,255,0.07)" : "none" }}>
      ...
      {/* ── SCROLL HINT ── */}
      <div
        className="relative flex flex-col items-center gap-2 py-4 pb-8 md:pb-4"
        style={{ zIndex: 1 }}
      >
